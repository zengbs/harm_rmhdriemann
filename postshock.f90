!$Id: postshock.f90,v 1.19 2005/08/16 13:54:44 bgiacoma Exp $
...
subroutine ContactVelocity(unk1,unk2,vxb,Vs,ahead,vx,switchLR,switchPB,solution)
  ...
  use interfaces,only: zeroeqn,postshock
  implicit none
  ...
  real(DP)::b2,pgas,wtot,leftAlfven,rightAlfven,cdvel,xmax,xmin,B2big,W2inv,eta
  ...
  wtot=ahead(1)+gamma*pgas/(gamma-1.0e0_dp)+b2 !total relativistic enthalpy
  ...
  if (veryverbose) print *,'ContactVelocity: f(xmax)=',f 
  if ((f==0.0e0_dp).AND.(.NOT.(errcheck))) then
     Vs=xmax
  ...
  if (veryverbose) print *,'ContactVelocity: f(xmin)=',f 
  if ((f==0.0e0_dp).AND.(.NOT.(errcheck))) then
     Vs=xmin
...
subroutine RHS_pressure(x,y,f,switchLR)
  ...
  h=(wtot-b2)/rho !specific gas enthalpy
...
subroutine alfven_func(ahead,Vs,behind,f)
  ...
  wtota=rhoa+gamma*(pa-0.5e0_dp*b2a)/(gamma-1)+b2a !total relativistic enthalpy
...
subroutine RHS_psi(x,y,f,switchLR)
  ...
  wtot = rho + gamma*(p-0.5e0_dp*b2small)/(gamma-1.0e0_dp)+b2small !total relativistic enthalpy
