!$Id: riemann_rmhd.f90,v 1.14 2005/08/16 14:18:53 bgiacoma Exp bgiacoma $
...
program riemann_rmhd
  ...
  use interfaces,only:initialdata,contact,output,fullcontact,xi
  ...
  !Giacomazzo & Rezzolla 2005, gr-qc/0507102, submitted to J. Fluid Mech.

  !Copyright (C) 2005  B. Giacomazzo, L. Rezzolla

  integer(I4B)::degen_case
  real(DP)::leftalfven_left,rightalfven_left,leftalfven_right,rightalfven_right,eta,b2,wtot
  real(DP),dimension(4)::all_left,all_right
  real(DP),dimension(4)::unk
  real(DP),PARAMETER::small_vel=epsilon(1.0e-10_dp)
  ...
  if(.NOT.(Bx==0.0e0_dp)) then
     ...
     wtot=left(1)+(left(2)-0.5e0_dp*b2)*gamma/(gamma-1.0e0_dp)+b2
     !right-going
     ...
     wtot=right(1)+(right(2)-0.5e0_dp*b2)*gamma/(gamma-1.0e0_dp)+b2
     !right-going
     ...
     if ((abs(leftalfven_left-all_left(1))<=small_vel).AND.&
        ...
        print *,'degeneravy ok kind 3, only SLOW WAVES'
        degen_case=3
        goto 3
        ...
        print *,'degeneravy ok kind 2, only FAST WAVES'
        degen_case=2
        degen=.true.
        goto 3
...
3 if ((Bx==0.0e0_dp).OR.(degen)) then
  ...
  print *
  print *,'the eight states of the exact solution obtained are written in file ''exact.sol'''
  print *,'solution written in file ''solution.dat'' at each iteration'
  print *,'Funcv (which gives the jumps at the CD of vx,vy,vz,By,Bz and p) is saved in ''funcv.dat'''
  print *,'Values at the Contact discontinuity and waves'' velocities saved in ''solution.sol'' at each iteration'
...
subroutine fullfuncv(left,right,unk,VsLv3,VsRv3,fvec,fullsolution)
  ...
  use interfaces,only: velocity,Rarefaction,alfven
  use interfaces,only: Rarefaction_pressure,ContactVelocity
  ...
  real(DP)::vx1,vy1,vz1,ptot1,vx2,vy2,vz2,ptot2,vxb,B2big,eta,W2inv,b2,wtot,normB,psi,dump
...
10 if (verbose) then
  ...
  wtot=zone1a(1)+(zone1a(2)-0.5e0_dp*b2)/(gamma-1.0e0_dp)+zone1a(2)+0.5e0_dp*b2 !Total relativistic enthalpy
...
11 if (verbose) then
  ...
  wtot=zone3b(1)+(zone3b(2)-0.5e0_dp*b2)/(gamma-1.0e0_dp)+zone3b(2)+0.5e0_dp*b2 !Total relativistic enthalpy
...
subroutine output(x1,x2,t,nx,left,right,VsLv3,VsRv3,unk,fullsolution)
  ...
  real(DP)::xc,vxc,pbc,h,pb,xir,xp,VsLeft,VsRight,normb
  real(DP),dimension(7,2)::solution
...
subroutine Rarefaction(normB,Vs,init,solution,switchLR)
  ...
  real(DP)::x,h
  integer(I4B)::j
  integer(I4B),PARAMETER::nsteps=1000 !number of steps
...
subroutine RHS(x,y,f,switchLR)
  ...
  wtot = rho + gamma*(p-0.5e0_dp*b2small)/(gamma-1.0e0_dp)+b2small

  !relativistic specific enthalpy
  h=(wtot-b2small)/rho

  !sound velocity squared
  cs2=gamma*(p-0.5e0_dp*b2small)/(wtot-b2small) !c_s^2=\Gamma*Pgas/w
...
subroutine xi(state,switchLR,Vs,allwaves)
  ...
  if (v2>1.0e0_dp) stop 'riemann.f90: xi: v2>1.0!'
  ...
  wtot=rho+Pgas/(gamma-1.0e0_dp)+Pgas+b2 !total relativistic enthalpy
  h=(wtot-b2)/rho !specific relativistic enthalpy
  cs2=gamma*Pgas/(wtot-b2) !c_s^2=\Gamma*P/w, sound velocity squared
  !-----------------------------------------------------!
